...
contract IDOPool is Ownable {
    ...
    uint256 public hardCap; // 6 * 10 ** 24
    uint256 public softCap; // 25 * 10 ** 23
    ...
    function purchase(uint256 _amount) external idoActive {
        ...
        require(
            usdc.balanceOf(msg.sender) >= _amount / rate / 10**12,
            "Insufficinet Fund"
        );
        usdc.transferFrom(msg.sender, address(this), _amount / rate / 10**12);
        totalUsdc += _amount / rate / 10**12;
        usdcAmount[msg.sender] += _amount / rate / 10**12;
